...
$(document).ready(function () {
    ...
    *************************************************************/
    ...
    $('#confirmSearch').click(function () {
        ...
        $('#mainContent').load(
            '../Web/BasicSetting/City.aspx',                              // 这里修改服务器提交位置
            {
...
// 添加或或修改数据行函数
function update() {
    if($('#cityName_edit').val().trim().length==0)                            // 这里修改控制判断语句
    {
        showError("请填写地市名称");
        return false;
    }
    ...
    $.ajax({
        type: "POST",
        url: "../Web/BasicSetting/City.aspx/update",                          // 这里要修改服务器提交位置
        data: data,
        ...
        success: function (json) {
            if (json.d == "1") {
                ...
                $('#mainContent').load('../Web/BasicSetting/City.aspx');      // 这里要修改服务器提交位置
                showSuccess("操作成功");
            }
...
function deleteModel(){
    ...
    $.ajax({
        type: "POST",
        url: "../Web/BasicSetting/City.aspx/deleteModel",       // 这里要修改服务器提交位置
        data: data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (json) {
